...
Add the following boilerplate code to the App component. This will add 'Sign Up', 'Sign In' and 'Sign Out' buttons and the functions they will call. As a result of each action, user's session (authentication state) will change so we create a state variable `session` to keep track of that.
